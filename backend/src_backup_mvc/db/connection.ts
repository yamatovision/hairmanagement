...
export const setupDatabaseConnection = (): void => {
  ...
  // 開発環境では常にDBエラーをスキップする
  process.env.SKIP_DB_ERRORS = process.env.NODE_ENV === 'development' ? 'true' : process.env.SKIP_DB_ERRORS;
  ...
  connectDB().catch(error => {
    ...
    if (!skipDBErrors) {
      logger.error('SKIP_DB_ERRORS=false のため致命的エラーとして扱います');
      process.exit(1);
    } else {
