...
export class MongoUserRepository extends BaseRepository<User, string> implements IUserRepository {
   ...
   */
  private mapRoleToUserRole(modelRole: string): User['role'] {
    switch (modelRole) {
   ...
   * @returns モデルのロール値
   */
  private mapUserRoleToModel(userRole: User['role']): string {
    switch (userRole) {
   ...
   * @returns ユーザーステータス
   */
  private determineUserStatus(isActive: boolean): User['status'] {
