/**
 * マッピングテーブルを使用した十神関係計算の検証テスト
 */

import { determineBranchTenGodRelation } from './tenGodFixedMapping';
...
const testCasesFromSamples: TestCase[] = [
  ...
  { description: "1970年 辛日 × 巳支", dayStem: "辛", branch: "巳", expected: "偏官" },
  { description: "1970年 辛日 × 子支", dayStem: "辛", branch: "子", expected: "傷官" },
  ...
  { description: "2015年 丁日 × 午支", dayStem: "丁", branch: "午", expected: "劫財" },
  ...
  // 前回のテストで不一致が見られた組み合わせを重点的にテスト
  { description: "不一致テスト 丙 × 寅", dayStem: "丙", branch: "寅", expected: "偏印" },
  { description: "不一致テスト 丁 × 卯", dayStem: "丁", branch: "卯", expected: "偏印" },
  { description: "不一致テスト 戊 × 辰", dayStem: "戊", branch: "辰", expected: "比肩" },
  { description: "不一致テスト 己 × 巳", dayStem: "己", branch: "巳", expected: "偏印" },
  { description: "不一致テスト 庚 × 午", dayStem: "庚", branch: "午", expected: "偏官" },
  { description: "不一致テスト 辛 × 未", dayStem: "辛", branch: "未", expected: "偏印" },
  { description: "不一致テスト 壬 × 戌", dayStem: "壬", branch: "戌", expected: "偏官" },
  { description: "不一致テスト 壬 × 辰", dayStem: "壬", branch: "辰", expected: "偏官" },
  { description: "不一致テスト 丁 × 午", dayStem: "丁", branch: "午", expected: "劫財" },
  { description: "不一致テスト 丁 × 子", dayStem: "丁", branch: "子", expected: "偏官" },
  { description: "不一致テスト 丁 × 丑", dayStem: "丁", branch: "丑", expected: "食神" },
];
...
const hiddenStemTestCases: TestCase[] = [
  ...
  // 丑（己辛癸）
  { description: "蔵干 甲日×丑支(己)", dayStem: "甲", branch: "丑", expected: "正財" },
  { description: "蔵干 甲日×丑支(辛)", dayStem: "甲", branch: "丑", expected: "正官" },
  ...
  { description: "蔵干 乙日×寅支(戊)", dayStem: "乙", branch: "寅", expected: "正財" },
];
...
function runAllTests() {
  console.log('マッピングテーブルによる十神関係計算テスト開始\n');
