...
function getLocalTimeAdjustedDate(date, options) {
    ...
    try {
        ...
        var timeDiffMinutes = (options.location.longitude - standardMeridian) * 4;
        // 3. 時差を分単位で調整
        ...
        switch (mode) {
            ...
            case 'korean':
                ...
                if (hours >= 0 && hours < 5) {
                    ...
                    // デバッグログ
                    console.log("\u65E5\u4ED8\u5909\u66F4\u30E2\u30FC\u30C9[".concat(mode, "]: ").concat(formatDateKey(date), " \u2192 ").concat(formatDateKey(prevDay), " (").concat(hours, "\u6642\u306F\u524D\u65E5\u6271\u3044)"));
                    // 新しい日付オブジェクトを返す
