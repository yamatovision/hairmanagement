...
import { ConversationOptions } from '../../services/unified-conversation.service';
...
/**
 * 統合会話用チャットインターフェース - 統合会話APIエンドポイントを使用
 */
const UnifiedChatInterface: React.FC<UnifiedChatInterfaceProps> = ({ 
  ...
  useEffect(() => {
    const initializeConversation = async () => {
      try {
        ...
        await startConversation(type, contextId, options);
      } catch (error) {
  ...
  }, [type, contextId, startConversation, options]);
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    // 会話タイプを指定して送信
    sendMessage(inputMessage, true, type, options);
    setInputMessage('');
  };
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <Box>
            ...
            <Typography variant="caption">Claude AI チャット</Typography>
          </Box>
        </Box>
